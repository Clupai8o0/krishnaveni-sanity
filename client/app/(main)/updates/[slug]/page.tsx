...
export async function generateMetadata({
	...
	const description =
		post.category === "gallery"
			? parseGalleryContent(post.content)?.description
...
function parseGalleryContent(content: string | null): GalleryContent | null {
...
export default async function PostPage({
	...
	const gallery = isGallery ? parseGalleryContent(post.content) : null;
	const youtubeId = post.video_url ? getYouTubeId(post.video_url) : null;
	const paragraphs = !isGallery
		? (post.content?.split("\n\n").filter(Boolean) ?? [])
		: [];

	return (
		<main className="max-w-5xl mx-auto px-4 md:px-8 pt-36 pb-24">
			<Link
				href="/updates"
