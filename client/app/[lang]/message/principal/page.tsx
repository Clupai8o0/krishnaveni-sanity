...
export async function generateMetadata({
	...
	const metadata = await getMetadata(lang, PAGE_TYPES.PRINCIPAL_MESSAGE);
	return metadata;
}

async function PrincipalMessagePage({
	params,
}: {
	params: Promise<PageParams>;
}) {
	const { lang } = await params;
	const data = await getMessagePageData(lang, PAGE_TYPES.PRINCIPAL_MESSAGE);
