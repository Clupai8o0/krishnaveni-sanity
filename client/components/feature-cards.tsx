...
import { FeatureCardsProps } from "@/lib/types";
...
const FeatureCard = ({ title, description, icon, color }: CardProps) => {
	return (
		...
		>
			...
			<h3 className="text-lg md:text-xl font-medium text-white text-center text-balance font-serif leading-tight">
				{title}
			</h3>
			<p className="text-sm md:text-base text-center text-pretty text-white opacity-80">
				{description}
			</p>
		</div>
	);
...
function FeatureCards({ cards }: FeatureCardsProps) {
	return (
		<div className="max-w-7xl mx-auto px-4 relative z-[12]">
			<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 relative -top-10 shadow-md rounded-lg overflow-hidden">
				{cards.map((card) => (
					<FeatureCard key={generateId()} {...card} />
				))}
			</div>
		</div>
	);
